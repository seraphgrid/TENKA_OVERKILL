 # tenko - a simple, preconfigured Emacs binding set for qutebrowser
# qutesheep is ultimately work on a (currently) seven-year-old script, qutemacs, uploaded by jumper047. 
...
config.source("themes/base16-horizon-term.config.py")
...
c.bindings.commands['normal'] = {
    ...
    # todo: copying the magnet link through through any uploaded media containing the link.
    ...
    '<ctrl-c><ctrl-l>': 'config-source ~/.config/qutebrowser/tenka.py ;; message-info "tenko configuration reloaded."',
    ...
    '<ctrl-u><a>': 'cmd-set-text :spawn -d /usr/bin/urxvt -e  yt-dlp --verbose -x --audio-quality 0 {url} -o $HOME/Music/%(title)s.%(ext)s  --audio-format flac ;; message-info "downloading current url as flac audio."', # current url video
    '<ctrl-u><v>': 'cmd-set-text :spawn -d /usr/bin/urxvt -e yt-dlp {url} -o $HOME/Videos/%(title)s.%(ext)s --verbose ', # current url audio
    ...
    '<ctrl-x><p><1>': 'set content.proxy system ;; message-info "system proxy. tenko see, tenko do."',
...
#    '<ctrl-x><shift-u>': 'redo', # this will exist eventually... right? 
    ...
    '<alt-w>': 'fake-key <Ctrl-c> ;; message-info "selection transposed to clipboard.', # 
...
#    '<ctrl-u><shift-t>': 'cmd-set-text :spawn -d terminator -x rtorrent "{primary}" -d ~/Downloads/', maybe... 
    ...
    '<ctrl-t><s>': 'config-cycle tabs.show always switching', 
...
c.bindings.commands['insert'] = {
    ...
    '<ctrl-c><ctrl-l>': 'config-source ~/.config/qutebrowser/tenko.py ;; message-info "tenko configuration reloaded."',
